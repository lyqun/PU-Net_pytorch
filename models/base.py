...
class Resnet_BaseBlock(nn.Module):
    def __init__(self, PNCONV,
        ...
        self.conv2 = PNCONV(npoint=npoint, 
                            nsample=nsample,
                            radius=radius,
    ...
    def forward(self, xyz, feats, npoint=None, new_xyz=None):
        assert (self.keep_pcd and new_xyz is None) or not self.keep_pcd, 'invalid new_xyz.'
        
        new_feats = self.conv1(feats)
        new_xyz, new_feats = self.conv2(xyz, new_feats, npoint=npoint, new_xyz=new_xyz)
        new_feats = self.conv3(new_feats)
        shc_feats = feats

        if self.keep_pcd and not self.is_im: # if in != out, apply an additional projection mlp
            shc_feats = self.sonv0(shc_feats) # mlp
        if not self.keep_pcd: # not keep pcd, apply pnconv with fps
